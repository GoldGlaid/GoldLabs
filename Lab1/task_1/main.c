...
int is_simple_digit(const long long int x) {
  if (x <= 0) {
    return 0;
  }
  for (int i = 2; i < x; ++i) {
...
// Функция перевода из строчного представления в числовое, проверка на некоторые
// условия
...
int main(int argc, char *argv[]) {
  ...
  // Проверка на то что, число не равно 0 (не для флага f, s)
  if (x == 0 && argv[1][1] != 'f' && argv[1][1] != 's') {
  ...
  // x. Если таковых нет – вывести соответствующее сообщение;
  if (argv[1][1] == 'h') {
    ...
    // Флаг -a вычислить сумму всех натуральных чисел от 1 до x и вывести
    // полученное значение в консоль;
  } else if (argv[1][1] == 'a') {
    long long int sum = 0;
